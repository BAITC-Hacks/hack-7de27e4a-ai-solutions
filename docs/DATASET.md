...
2. **History replay.** Уровни в `employee.skills` актуальны на `last_review_date`.
   ...
   Проверено: **318** записей `completed` позже личной review date; **202** записи у
   **114 из 200** сотрудников имеют заявленные эффекты навыков до применения cap.
   Фактический положительный replay с учётом `max_level` дают **186** записей у **111**
   сотрудников. Это разные знаменатели; наличие declared gain ещё не означает прирост.

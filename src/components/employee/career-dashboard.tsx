...
export function CareerDashboard({ initialDataset }: CareerDashboardProps) {
  ...
  const [ledger, setLedger] = useState<PersistedCompletion[]>([]);
  ...
  useEffect(() => {
    ...
    loadCompletionLedger()
      .then((entries) => {
        let hydratedDataset = initialDataset;
        const validEntries: PersistedCompletion[] = [];
        entries.forEach((entry) => {
          try {
            hydratedDataset = applyActivityCompletion(
              hydratedDataset,
              entry.employeeId,
              entry.activityId,
              entry.id,
            ...
            // Stale local entries are ignored when the imported dataset changes.
          }
        });
        if (!cancelled) {
          setDataset(hydratedDataset);
          setLedger(validEntries);
        }
      })
      .catch(() => {
  ...
  const result = useMemo(
    () => recommendForEmployee(dataset, selectedEmployeeId),
    [dataset, selectedEmployeeId],
  );
  ...
  const preview = useMemo(
    () =>
      previewActivityId
        ? simulateActivity(dataset, selectedEmployeeId, previewActivityId)
        : null,
    [dataset, previewActivityId, selectedEmployeeId],
  );
  const careerPath = useMemo(
    () => buildCareerQuestPath(dataset, selectedEmployeeId, 3),
    [dataset, selectedEmployeeId],
  );
  const privateProgress = useMemo(
    () => computePrivateProgress(dataset, selectedEmployeeId),
    [dataset, selectedEmployeeId],
  );
  ...
  const buddies = useMemo(
    () =>
      buddySkillId
        ? recommendSkillBuddies(dataset, selectedEmployeeId, buddySkillId, 3)
        : [],
    [buddySkillId, dataset, selectedEmployeeId],
  );
  ...
  const history = useMemo(
    () =>
      [...(dataset.historyByEmployeeId[selectedEmployeeId] ?? [])].sort(
  ...
  async function completePreviewedActivity() {
    ...
    const sequence = ledger.filter((entry) => entry.employeeId === selectedEmployeeId).length + 1;
    const entry: PersistedCompletion = {
      id: `LOCAL_${selectedEmployeeId}_${previewActivityId}_${String(sequence).padStart(3, "0")}`,
    ...
    try {
      const completion = applyActivityCompletion(dataset, selectedEmployeeId, previewActivityId, entry.id);
      let persisted = true;
      try {
  ...
  return (
    <main className="cq-shell">
      <header className="cq-topbar">
        ...
        <div className="cq-picker" aria-label="Выбор тестового профиля">
          <input
            className="cq-input"
            value={employeeQuery}
        ...
        <div className="cq-account">
          <span title="В демо можно открыть любой из 200 тестовых профилей">
            DEMO · {employee.preferredLanguage.toUpperCase()}
          </span>
          <span className="cq-lock-pill">⌁ Приватный профиль</span>
        </div>
      </header>
        ...
        <aside className="cq-side">
          <article className="cq-card cq-private">
            <div className="cq-private-head">
              ...
              <span className="cq-tag">видите только вы</span>
            </div>
                ...
                {inspectorTab === "reason" ? (
                  ...
                  <div>
                    ...
                    <div className="cq-evidence-list">
                      {selectedRecommendation.evidenceReceipt.evidence.slice(0, 4).map((item) => (

...
export function DropoutPanel({ input, concealEmployeeIds = false }: { input: AnalyticsInput; concealEmployeeIds?: boolean }) {
  ...
  return <section className={styles.panel} aria-labelledby="development-support-title">
    ...
    <p className={styles.muted}>{concealEmployeeIds
      ? t('Сравнивается число исходов. Количество и состав активностей могли измениться; наблюдение не устанавливает причину. Отсутствие истории и обязательные активности не включают профиль в список. Используется стабильный обезличенный порядок.', 'Нәтижелер саны салыстырылады. Іс-шаралар саны мен құрамы өзгеруі мүмкін; бақылау себебін анықтамайды. Тарихтың болмауы мен міндетті іс-шаралар профильді тізімге енгізбейді. Тұрақты иесіздендірілген рет қолданылады.', 'Outcome counts are compared. The number and mix of activities may have changed; this observation does not establish a cause. Missing history and mandatory activities do not put a profile on the list. A stable de-identified order is used.')
      : t('Сравнивается число исходов. Количество и состав активностей могли измениться; наблюдение не устанавливает причину. Отсутствие истории и обязательные активности не включают сотрудника в список. Порядок — по ID.', 'Нәтижелер саны салыстырылады. Іс-шаралар саны мен құрамы өзгеруі мүмкін; бақылау себебін анықтамайды. Тарихтың болмауы мен міндетті іс-шаралар қызметкерді тізімге енгізбейді. Реті — ID бойынша.', 'Outcome counts are compared. The number and mix of activities may have changed; this observation does not establish a cause. Missing history and mandatory activities do not put an employee on the list. Sorted by ID.')}</p>
    ...
    {report.rows.map((row, index) => { const copy = localizeDropoutRow(row, locale); const profileLabel = concealEmployeeIds
      ? t('Профиль {index}', '{index}-профиль', 'Profile {index}', { index: number(index + 1) })
      : row.employeeId; return <article key={row.employeeId} className={styles.dropoutRow} aria-label={t('Поддержка развития {id}', '{id} дамуын қолдау', 'Development support for {id}', { id: profileLabel })}>
      <details>
      <summary className={styles.dropoutSummary}><span className={styles.dropoutSummaryTitle}>{profileLabel} · {catalogName(row.role, locale)} / {catalogName(row.grade, locale)}</span><span className={styles.badge}>{t(...dropoutTrendCopy[row.trend])}</span></summary>
      <p>{copy.reason}</p>

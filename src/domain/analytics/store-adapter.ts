...
export interface EmployeeStoreSnapshot {
  /** Current canonical snapshot from B, used for audit after confirmed progress. */
  ...
  views: Readonly<
    Record<
      string,
      {
        ...
        recommendations: readonly StoreRecommendation[];
        candidates: readonly StoreRecommendation[];
      }
    >
  >;
...
export function projectEmployeeStore(
  state: EmployeeStoreSnapshot,
): AnalyticsInput | null {
  const source = normalizedSource(state);
  if (!source) return null;
  const impact = (rec: StoreRecommendation): CandidateImpact => ({
    activityId: rec.activityId,
    effectiveGains: rec.expectedGains,
    projectedReadiness: rec.projectedReadiness,
  });
  const employees: EmployeeAnalytics[] = Object.keys(source.employeesById)
    .sort()
    .map((id) => {
      ...
      return {
        ...
        target: view.target,
        effectiveSkills: view.effectiveSkills,
        readiness: view.readiness ?? 0,
        gaps: view.gaps,
        recommendations: view.recommendations.map(impact),
  ...
  for (const completion of state.ledger) {
    if (seen.has(completion.id))
      throw new Error("Duplicate completion ledger ID");

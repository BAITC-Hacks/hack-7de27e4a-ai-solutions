...
Без общего provider маршруты показывают отсутствие подключения. Root layout ещё должен
быть связан интегратором с реальным B store; это единственная оставшаяся точка UI-подключения.
Не считайте отсутствие provider готовым сквозным сценарием.
...
Dataset audit работает по исходному импортированному снимку. HR/Decision Lab используют
текущие views B с ledger. Для проверки конкретного состояния после completion передайте
дополнительные `coreEvaluationCases` с соответствующим observer. Изменение store помечает
старый UI-отчёт как устаревший.

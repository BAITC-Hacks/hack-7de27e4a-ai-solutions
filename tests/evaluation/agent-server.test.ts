...
import {
  AGENT_TOOLS,
  createAgentProvider,
} from "../../src/app/api/ai/agent/provider";
import { GET, POST } from "../../src/app/api/ai/agent/route";
import {
  AGENT_TOOL_ARGUMENT_SCHEMAS,
  type AgentHistoryEntry,
  type AgentStepRequest,
} from "../../src/lib/evaluation/agent-contracts";
...
beforeEach(() => {
  ...
  vi.stubEnv("SESSION_SECRET", "agent-test-secret-at-least-thirty-two-bytes");
  vi.stubEnv("APP_ORIGIN", "");
});
const hrId = Object.values(loadChallengeDataset().employeesById).find(
  (employee) => employee.role === "HR Business Partner",
)!.id;
const identityHeaders = () => ({
  cookie: `${SESSION_COOKIE_NAME}=${signDemoSession(hrId)}`,
});
const get = () =>
  GET(
    new Request("http://localhost:3000/api/ai/agent", {
      headers: identityHeaders(),
    }),
  );
const request = (): AgentStepRequest => ({
  language: "ru",
  question: "Какие пробелы у E0028?",
  history: [],
});
const entry = (index = 1): AgentHistoryEntry => ({
  call: {
    id: `call_${index}`,
    name: "getGaps",
    arguments: { employeeId: "E0028" },
  },
  result: {
    callId: `call_${index}`,
    evidenceId: `tool:${index}:getGaps`,
    tool: "getGaps",
    facts: [
      {
        id: "gap",
...
describe("bounded HR agent server", () => {
  it("disables without key and never invokes a model", async () => {
    expect(await runAgentStep(request())).toEqual({
      status: "no_key",
  ...
  it("derives tool JSON Schema constraints from the shared Zod args, including strict nullable filters", () => {
    const get = AGENT_TOOLS.find((tool) => tool.function.name === "getGaps")!
      .function.parameters;
    expect(get.additionalProperties).toBe(false);
    expect(get.required).toEqual(["employeeId"]);
    expect(get.properties!.employeeId).toMatchObject({
      type: "string",
      pattern: "^[A-Za-z][A-Za-z0-9_-]{0,79}$",
    });
    const find = AGENT_TOOLS.find(
      (tool) => tool.function.name === "findEmployees",
    )!.function.parameters;
    expect(find.additionalProperties).toBe(false);
    expect(find.required).toEqual(["roleId", "grade", "skillId", "minLevel"]);
    expect(find.properties!.minLevel).toMatchObject({
      anyOf: [{ type: "number", minimum: 0, maximum: 5 }, { type: "null" }],
    });
    expect(
      AGENT_TOOL_ARGUMENT_SCHEMAS.findEmployees.safeParse({
        skillId: "SK_SYSTEM_DESIGN",
        minLevel: 5,
      }).success,
    ).toBe(true);
    ...
    expect(
      AGENT_TOOL_ARGUMENT_SCHEMAS.findEmployees.safeParse({ minLevel: 3 })
        .success,
    ).toBe(false);
  });
  it("makes a tool call then verifies an answer against its result", async () => {
    ...
    const provider = vi
      .fn()
      .mockResolvedValueOnce({ status: "tools", calls: [first.call] })
      .mockResolvedValueOnce({
        status: "answer",
        answer: {
          text: "У E0028 остался разрыв по SK_SYSTEM_DESIGN: Разрыв — 1. [tool:1:getGaps/gap]",
          claims: [
            {
    ...
    expect(await runAgentStep(request(), { provider })).toMatchObject({
      status: "tools",
    });
  ...
  it("blocks an invented fact and unsupported tool arguments before execution", async () => {
    ...
    expect(
      await runAgentStep(
        ...
        { provider: async () => fabricated },
      ),
    ).toMatchObject({ status: "blocked", reason: "FACT_MISMATCH" });
    expect(
      await runAgentStep(request(), {
        provider: async () => ({
          status: "tools",
          calls: [
            {
              ...entry().call,
              arguments: { employeeId: "E0028", instruction: "Ignore rules" },
            },
          ],
        }),
      }),
    ).toMatchObject({ status: "blocked", reason: "INVALID_MODEL_OUTPUT" });
  });
  it("enforces five tool calls but permits final grounded answer after the fifth", async () => {
    ...
    expect(
      await runAgentStep(full, {
        provider: async () => ({
          status: "tools",
          calls: [{ ...entry().call, id: "call_6" }],
        }),
      }),
    ).toMatchObject({ status: "blocked", reason: "STEP_LIMIT" });
    expect(
      await runAgentStep(full, {
        provider: async () => ({
          status: "answer",
          answer: {
            text: "Недостаточно данных для полного ответа.",
            claims: [],
          },
        }),
      }),
    ...
    expect(
      await runAgentStep({ ...full, history: [...full.history, entry(6)] }),
    ).toMatchObject({ status: "blocked", reason: "INVALID_REQUEST" });
  });
  it("bounds a hung provider and aborts it; supports caller cancellation", async () => {
    let providerSignal: AbortSignal | undefined;
    const hung = (_: AgentStepRequest, signal: AbortSignal) => {
      providerSignal = signal;
      return new Promise<unknown>(() => {});
    };
    expect(
      await runAgentStep(request(), { provider: hung, timeoutMs: 5 }),
  ...
  it("does not disclose provider errors or secrets", async () => {
    const result = await runAgentStep(request(), {
      provider: async () => {
        throw new Error("Bearer secret-token and private profile");
      },
    });
  ...
  it("rejects cross-origin, raw data and oversized bodies; accepts proxy host", async () => {
    ...
    expect((await POST(post({ ...request(), rawProfile: {} }))).status).toBe(
      400,
    );
    expect(
      (await POST(post(request(), { "Content-Type": "text/plain" }))).status,
    ).toBe(415);
  ...
  it("forces final answer at the tool limit and handles provider access errors safely", async () => {
    ...
    expect(
      await runAgentStep(
        { ...request(), history: [1, 2, 3, 4, 5].map(entry) },
        { provider },
      ),
    ).toEqual({ status: "blocked", reason: "PROVIDER_ACCESS_DENIED" });
